...
(and (string-match-p "[A-Z]"))
(if (and (string-match "[A-Z]" sentence)
         (if (string-match "[a-z]" sentence) nil t))
    t
  nil)

(defun isShouted (sentence)
  "SENTENCE is a string."
  (let (case-fold-search)
    (and (string-match "[A-Z]" sentence)
         (not (string-match "[a-z]" sentence)))))
